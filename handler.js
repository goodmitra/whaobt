...
var ucapanWaktu = 'Good night'
        ...
        Client.cmd.on('addvn', async (data) => {
            ...
            data.reply(`Ⴤᗝυᖇ ᗩυᖱ⫯𝖮 ƝᎯⲙ∈${data.body} 🇦​​​​​🇩​​​​​🇩​​​​​ 🇸​​​​​🇺​​​​​🇨​​​​​🇨​​​​​🇪​​​​​🇸​​​​​🇸​​​​​🇫​​​​​🇺​​​​​🇱​​​​​🇱​​​​​🇾​​​​​`)
        })
        ...
        Client.cmd.on('listvn', async (data) => {
            let listvn = '𝐘𝐎𝐔𝐑 𝐕𝐍 𝐀𝐔𝐃𝐈𝐎 𝐋𝐈𝐒𝐓 𝐉𝐔𝐒𝐓 𝐓𝐘𝐏𝐄 𝐓𝐇𝐈𝐒 :\n\n*List vn*:\n\n'
        ...
        Client.cmd.on('owner', async (data) => {
            Client.sendContact(data.from, { number: configs.ownerList[0].split('@')[0], name: 'DOC-2' }, data.message)
        })
	    ...
	    Client.cmd.on('song', async (data) =>{
			...
			let po = client.prepareMessageFromContent(data.from, {
				  "listMessage":{
                  "title": "*𝕐𝕠𝕦𝕋𝕦𝕓𝕖✪𝔻𝕠𝕨𝕟𝕝𝕠𝕒𝕕   ⓒ*",
                  "description": `*𝖘𝖊𝖆𝖗𝖈𝖍 𝖞𝖆𝖒𝖊✪: ${data.body}*\n*𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝 𝐕𝐢𝐝𝐞𝐨 𝐁𝐲 𝐂𝐥𝐢𝐜𝐤 𝐁𝐮𝐭𝐭𝐨𝐧 𝐁𝐞𝐥𝐥𝐨𝐰*`,
                  "buttonText": "𝙎𝙚𝙡𝙚𝙘𝙩✪𝙏𝙮𝙥𝙚 𝙈𝙋⓷✪𝙈𝙋⓸",
                  "listType": "SINGLE_SELECT",
                  "sections": secs}}, {}) 
            client.relayWAMessage(po, {waitForAck: true})	
			})
...
`
        ...
        Client.cmd.on('missing', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                Client.sendFileFromBase64(data.from, getAxios.data.toString('base64'), 'missing.jpg', '*𝕭𝖄:@𝖉𝖔𝖈𝖊𝖉𝖎𝖙𝖔𝖗𝖒𝖒𝖝* ', data.message)
                ...
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/missing-image?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}&text1=${text[0]}&text2=${text[1]}&text3=${text[2]}`, 'missing.jpg', '*𝕭𝖄:@𝖉𝖔𝖈𝖊𝖉𝖎𝖙𝖔𝖗𝖒𝖒𝖝* ', data.message)
        ...
        Client.cmd.on('calender', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                Client.sendFileFromBase64(data.from, getAxios.data.toString('base64'), 'p.jpg', '*𝕭𝖄:@𝖉𝖔𝖈𝖊𝖉𝖎𝖙𝖔𝖗𝖒𝖒𝖝* ', data.message)
            } else if(data.mentionedJidList.length > 0) {
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/calender?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', '*𝕭𝖄:@𝖉𝖔𝖈𝖊𝖉𝖎𝖙𝖔𝖗𝖒𝖒𝖝* ', data.message)
        ...
        Client.cmd.on('removebg', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/removebg?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', '*𝕭𝖄:@𝖉𝖔𝖈𝖊𝖉𝖎𝖙𝖔𝖗𝖒𝖒𝖝* ', data.message)
            } else data.reply(`Wrong format!, tag someone or reply image with ${data.prefix}calender`)
        ...
        Client.cmd.on('drawing', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/draw-image?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', '*𝕭𝖄:@𝖉𝖔𝖈𝖊𝖉𝖎𝖙𝖔𝖗𝖒𝖒𝖝* ', data.message)
        ...
        Client.cmd.on('sketch', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/sketch-image?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', '*𝕭𝖄:@𝖉𝖔𝖈𝖊𝖉𝖎𝖙𝖔𝖗𝖒𝖒𝖝* ', data.message)
            } else data.reply(`Wrong format!, tag someone or reply image with ${data.prefix}drawing`)
...
RAM : ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB
             ...
             █░▀░█ █░▀░█ █░█`
                ...
                case 'nulis':
                    ...
                    data.reply(mess.wait)
                    Client.sendFileFromUrl(from, `${configs.apiUrl}/api/${command}?text=${data.body}&apikey=${configs.zeksKey}`, 'gambar.jpg', `*𝕭𝖄:@𝖉𝖔𝖈𝖊𝖉𝖎𝖙𝖔𝖗𝖒𝖒𝖝* @${data.sender.split('@')[0]}`, message)
                    } catch {
                        data.reply('error')
                    }
                    break
                ...
                case 'watercolour':
                    ...
                    p = data.body
                    text = p.split('|')
                    Client.sendFileFromUrl(from, `${configs.apiUrl}/api/${command}?apikey=${configs.zeksKey}&text1=${text[0]}&text2=${text[1]}`, 'p.jpg', `*𝕭𝖄:@𝖉𝖔𝖈𝖊𝖉𝖎𝖙𝖔𝖗𝖒𝖒𝖝* @${data.sender.split('@')[0]}`, message)
                    } catch {
                        data.reply('error')
                    }
					break
                ...
                case 'hartatahta':
                    ...
                    if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}${data.command} [ teks ]*\nContoh : ${data.prefix}${data.command} shiro`)
                    data.reply(mess.wait)
                    Client.sendFileFromUrl(from, `${configs.apiUrl}/api/hartatahta?text=${data.body}&apikey=${configs.zeksKey}`, 'harta.jpg', `*𝕭𝖄:@𝖉𝖔𝖈𝖊𝖉𝖎𝖙𝖔𝖗𝖒𝖒𝖝* @${data.sender.split('@')[0]}`, message)
